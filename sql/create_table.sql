...
-- 创建 SQL 编辑器文件表
CREATE TABLE IF NOT EXISTS `data_studio_sql_edit` (
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='SQL 编辑器文件表';

-- 创建 数据摄取文件表
CREATE TABLE IF NOT EXISTS `data_studio_data_ingestion` (
    `id` bigint NOT NULL AUTO_INCREMENT COMMENT '文件ID',
    `name` varchar(100) NOT NULL COMMENT '文件/文件夹名称',
    `type` varchar(20) NOT NULL DEFAULT 'file' COMMENT '文件类型：folder-文件夹，yaml-yaml文件，file-普通文件',
    `parent_id` bigint DEFAULT '0' COMMENT '父文件夹ID',
    `file_path` varchar(500) DEFAULT NULL COMMENT '文件路径',
    `content` longtext COMMENT '文件内容（仅对文件有效，文件夹为空）',
    `sort` int NOT NULL DEFAULT '0' COMMENT '显示顺序',
    ...
    KEY `idx_type` (`type`) USING BTREE,
    KEY `idx_name` (`name`) USING BTREE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='数据摄取文件表';
...
VALUES ('data_studio_sql_edit_seq', 0, 1);

-- 初始化 数据摄取序列号
INSERT IGNORE INTO `test_sequences` (`sequence_name`, `current_value`, `increment`)
VALUES ('data_studio_data_ingestion_seq', 0, 1);
