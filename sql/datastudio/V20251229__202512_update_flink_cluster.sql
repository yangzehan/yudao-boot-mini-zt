...
alter table data_studio_flink_cluster
    add last_heartbeat_time datetime null comment '最后一次心跳时间' after custom_config;

-- 更新现有Yarn集群测试数据，设置配置文件路径默认值
UPDATE `data_studio_flink_cluster`
SET `yarn_site_path` = '/etc/hadoop/conf/yarn-site.xml',
...
UPDATE `data_studio_flink_cluster`
SET `jobmanager_memory_process_size`   = 1600,
    `taskmanager_memory_process_size`  = 4096,
    `taskmanager_number_of_task_slots` = 2
WHERE `type` = 'yarn'
  AND (`jobmanager_memory_process_size` IS NULL OR `taskmanager_memory_process_size` IS NULL OR
       `taskmanager_number_of_task_slots` IS NULL);
