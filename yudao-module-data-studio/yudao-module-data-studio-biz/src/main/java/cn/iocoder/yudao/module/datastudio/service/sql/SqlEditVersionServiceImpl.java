...
public class SqlEditVersionServiceImpl implements SqlEditVersionService {
  ...
  @Resource private SqlEditVersionMapper versionMapper;
  ...
  public Long createVersion(
      Long sqlEditId, String content, FlinkConfig config, String remark, String versionType) {
    // 1. 验证文件是否存在
  ...
  public PageResult<SqlEditVersionDO> getVersionList(Long sqlEditId, PageParam pageParam) {
    return versionMapper.selectPageBySqlEditId(
        sqlEditId, pageParam.getPageNo(), pageParam.getPageSize());
  }
  ...
  public Boolean rollbackToVersion(Long versionId) {
    ...
    createVersion(
        ...
        version.getContent(),
        version.getConfig(),
        "回退到版本 v" + version.getVersionNumber(),
        "manual");
  ...
  public Integer deleteOldVersions(Long sqlEditId, Integer keepCount) {
    ...
    // 3. 获取需要删除的版本列表（最旧的那些）
    List<SqlEditVersionDO> versionsToDelete =
