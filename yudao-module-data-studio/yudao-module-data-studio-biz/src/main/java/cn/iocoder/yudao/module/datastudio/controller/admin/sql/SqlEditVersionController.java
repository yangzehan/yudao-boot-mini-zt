...
import cn.iocoder.yudao.module.datastudio.controller.admin.sql.vo.version.*;
import cn.iocoder.yudao.module.datastudio.dal.dataobject.file.SqlEditVersionDO;
import cn.iocoder.yudao.module.datastudio.service.sql.SqlEditVersionService;
...
public class SqlEditVersionController {
  ...
  @Resource private SqlEditVersionService versionService;

  @Operation(summary = "获取版本列表")
  @GetMapping("/list")
  public CommonResult<PageResult<SqlEditVersionRespVO>> getVersionList(
      @Valid SqlEditVersionListReqVO reqVO) {
    PageResult<SqlEditVersionDO> pageResult =
        versionService.getVersionList(reqVO.getSqlEditId(), reqVO);
    List<SqlEditVersionRespVO> voList =
        pageResult.getList().stream().map(SqlEditVersionRespVO::of).collect(Collectors.toList());
