...
public class DataIngestionServiceImpl implements DataIngestionService {
  ...
  @Resource private DataIngestionMapper dataIngestionMapper;

  @Resource private DataIngestionVersionService dataIngestionVersionService;

  @Resource private DataIngestionConfigMapper dataIngestionConfigMapper;
  ...
  private void initDefaultRootFolder() {
    ...
    if (!rootFiles.isEmpty()
        && rootFiles.stream().anyMatch(f -> tenantId.equals(f.getTenantId()))) {
  ...
  public void saveFileData(DataIngestionDataSaveReqVO saveReqVO) {
    ...
    dataIngestionVersionService.createVersion(
        saveReqVO.getId(), newContent, currentConfig, "自动保存版本", "auto");
  }
  ...
  public FlinkConfig getFileConfig(Long dataIngestionId) {
    DataIngestionConfigDO configDO =
        dataIngestionConfigMapper.selectByDataIngestionId(dataIngestionId);
    if (configDO == null) {
      return null;
    }
  ...
  public void saveFileConfig(Long dataIngestionId, FlinkConfig config) {
    // 查询是否已存在配置
    DataIngestionConfigDO existingConfig =
        dataIngestionConfigMapper.selectByDataIngestionId(dataIngestionId);
  ...
  public String deploy(Long id) {
    DataIngestionDO dataIngestionDO = dataIngestionMapper.selectById(id);
    Assert.notNull(dataIngestionDO, "文件不存在");

    DataIngestionConfigDO dataIngestionConfigDO =
        dataIngestionConfigMapper.selectByDataIngestionId(id);
    Assert.notNull(dataIngestionConfigDO, "文件配置不存在");

    FlinkConfig flinkConfig =
        deployUtil.getFlinkConfigByDataIngestionDOAndDataIngestionConfigDO(
            dataIngestionDO, dataIngestionConfigDO);

    FlinkApi flinkApi = FlinkApiFactory.getFlinkApiByVersion(flinkConfig.getFlinkVersion());

    JobDeployDataIngestionReqDto reqDto =
        JobDeployDataIngestionReqDto.builder()
            .content(dataIngestionDO.getContent())
            .flinkConfig(flinkConfig)
            .clusterId(flinkConfig.getClusterId())
            .jobName(dataIngestionDO.getName())
            .fileId(dataIngestionDO.getId())
            .flinkCdcDistJarPath(flinkConfig.getFlinkCdcDistJarPath())
            .build();
    JobDeployRespDto data = flinkApi.deployDataIngestion(reqDto).getCheckedData();
    ...
    FlinkJobDeployDO deployDO =
        FlinkJobDeployDO.builder()
            ...
            .flinkVersion(flinkConfig.getFlinkVersion())
            .status(JobStatus.RUNNING)
            .jobType(JobTypeEnum.DATA_INGESTION)
