...
public class YarnClusterDescriptor implements ClusterDescriptor<ApplicationId> {
  ...
  /**
   * The class to start the application master with. This class runs the main method in case of the
   * job cluster.
   */
  ...
  private ApplicationReport startAppMaster(
    ...
    if (YarnApplicationClusterEntryPoint.class.getName().equals(yarnClusterEntrypoint)
      ...
      fileUploader.registerMultipleLocalResources(
          Collections.singletonList(new Path(PackagedProgramUtils.getPythonJar().toURI())),
    ...
    if (System.getenv("IN_TESTS") != null) {
      ...
      if (System.getProperty("java.security.krb5.conf") != null) {
        configuration.set(
            SecurityOptions.KERBEROS_KRB5_PATH, System.getProperty("java.security.krb5.conf"));
      }
    }
  ...
  private void setTokensFor(ContainerLaunchContext containerLaunchContext, boolean fetchToken)
    ...
    Collection<Token<? extends TokenIdentifier>> usrTok = currUsr.getCredentials().getAllTokens();
  ...
  /**
   * Returns the configured remote target home directory if set, otherwise returns the default home
   * directory.
   *
  ...
  public String getClusterDescription() {
    ...
    try {
      ...
      ps.println(
          "Summary: totalMemory " + getDisplayMemory(totalMemory) + " totalCores " + totalCores);
    ...
    ClusterEntrypointUtils.tryFindUserLibDirectory()
        .ifPresent(
            usrLibDirFile -> {
              ...
              LOG.info(
                  "usrlib: {} will be shipped automatically.", usrLibDirFile.getAbsolutePath());
            });
  }
  ...
  Map<String, String> generateApplicationMasterEnv(
    ...
    env.put(YarnConfigKeys.FLINK_YARN_FILES, fileUploader.getApplicationDir().toUri().toString());
    // https://github.com/apache/hadoop/blob/trunk/hadoop-yarn-project/hadoop-yarn/hadoop-yarn-site/src/site/markdown/YarnApplicationSecurity.md#identity-on-an-insecure-cluster-hadoop_user_name
    env.put(
        YarnConfigKeys.ENV_HADOOP_USER_NAME, UserGroupInformation.getCurrentUser().getUserName());
   ...
   * ApplicationSubmissionContext} supports various methods which, depending on the Hadoop version,
  ...
  private static class ApplicationSubmissionContextReflector {
    ...
    private ApplicationSubmissionContextReflector(Class<ApplicationSubmissionContext> clazz) {
      ...
      try {
        ...
        LOG.debug(
            "{} does not support method {}.", clazz.getCanonicalName(), NODE_LABEL_EXPRESSION_NAME);
