...
public final class Utils {
    ...
    if (!StringUtils.isNullOrWhitespaceOnly(applicationFilesDir)) {
      ...
      try {
        ...
        if (!fileSystem.delete(path, true)) {
          LOG.error(
              "Deleting yarn application files under {} was unsuccessful.", applicationFilesDir);
        }
  ...
  static ContainerLaunchContext createTaskExecutorContext(
    ...
    decodeYarnLocalResourceDescriptorListFromString(shipListString)
        .forEach(
            resourceDesc ->
                taskManagerLocalResources.put(
                    resourceDesc.getResourceKey(), resourceDesc.toLocalResource()));
  ...
  private static List<YarnLocalResourceDescriptor> decodeYarnLocalResourceDescriptorListFromString(
  ...
  /**
   * Sets the application ACLs for the given ContainerLaunchContext based on the values specified in
   * the given Flink configuration. Only ApplicationAccessType.VIEW_APP and
   * ApplicationAccessType.MODIFY_APP ACLs are set, and only if they are configured in the Flink
   * configuration. If the viewAcls or modifyAcls string contains the WILDCARD_ACL constant, it will
   * replace the entire string with the WILDCARD_ACL. The resulting map is then set as the
