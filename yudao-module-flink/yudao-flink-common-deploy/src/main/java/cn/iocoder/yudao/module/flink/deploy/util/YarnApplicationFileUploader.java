...
public class YarnApplicationFileUploader implements AutoCloseable {
  ...
  Tuple2<Path, Long> uploadLocalFileToRemote(final Path localSrcPath, final String relativeDstPath)
      throws IOException {
      ...
      if (Utils.isRemotePath(shipFile.toString())) {
        if (fileSystem.isDirectory(shipFile)) {
          ...
          while (iterable.hasNext()) {
            ...
            relativePaths.add(
                new Path(localResourcesDirectory, parentURI.relativize(current.toUri()).getPath()));
          }
          continue;
        }
      } else {
        ...
        if (file.isDirectory()) {
          ...
          for (java.nio.file.Path javaPath : paths) {
            ...
            relativePaths.add(
                new Path(localResourcesDirectory, parentPath.relativize(javaPath).toString()));
          }
          continue;
        }
      }
      ...
      if (!isFlinkDistJar(relativePath.getName())) {
        ...
        final YarnLocalResourceDescriptor resourceDescriptor =
            registerSingleLocalResource(
                key, localPath, relativePath.getParent().toString(), resourceType, true, true);
  ...
  public YarnLocalResourceDescriptor uploadFlinkDist(final Path localJarPath)
    ...
    flinkDist =
        registerSingleLocalResource(
            localJarPath.getName(), localJarPath, "", LocalResourceType.FILE, true, false);
    return flinkDist;
  }
  ...
  List<String> registerProvidedLocalResources() {
    ...
    providedSharedLibs.forEach(
        (fileName, fileStatus) -> {
          ...
          final YarnLocalResourceDescriptor descriptor =
              YarnLocalResourceDescriptor.fromFileStatus(
                  fileName, fileStatus, LocalResourceVisibility.PUBLIC, LocalResourceType.FILE);
