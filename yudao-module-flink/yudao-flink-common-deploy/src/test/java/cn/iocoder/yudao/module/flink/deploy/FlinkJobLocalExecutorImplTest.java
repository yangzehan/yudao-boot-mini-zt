...
class FlinkJobLocalExecutorImplTest {
   ...
   * 测试执行Flink SQL的功能
   *
   * <p>该测试方法验证Flink本地执行器能否正确执行SQL语句。 主要包括创建数据源表(datagen_source)、创建输出表(print_sink)以及执行数据插入操作。
   *
   * <p>测试流程： 1. 创建Flink本地执行器实例 2. 构造SQL执行参数，包括配置信息和SQL语句 3. SQL语句包含创建源表、创建打印输出表和数据插入操作 4.
   * 调用执行器执行SQL并获取结果 5. 打印执行结果
   *
   ...
   * 测试执行JAR包的功能
   *
   * <p>该测试方法验证Flink本地执行器能否正确执行指定的JAR文件。 主要包括设置执行配置、指定JAR文件路径等操作。
   *
  ...
  void executeJar() {
    ...
    // 设置要执行的JAR文件路径
    deployLocalJarParam.setJarFile(
    ...
    // 执行JAR包
    flinkJobLocalExecutor.deployJar(deployLocalJarParam, true);
  }
}
