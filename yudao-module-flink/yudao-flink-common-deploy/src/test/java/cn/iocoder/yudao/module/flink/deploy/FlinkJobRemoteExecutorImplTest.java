...
class FlinkJobRemoteExecutorImplTest {
     ...
     * 测试远程执行Flink SQL的功能
     * <p>
     * 该测试方法验证Flink远程执行器能否通过REST API正确提交并执行SQL语句。
     * 主要包括创建数据源表(datagen_source)、创建输出表(print_sink)以及执行数据插入操作。
     * <p>
     * 测试流程：
     * 1. 构造远程SQL执行参数，包括连接配置和SQL语句
     * 2. 设置远程Flink集群的REST服务地址和端口
    ...
    void executeSql() {
        ...
        executeParam.setSql("-- 创建一个使用 datagen 连接器的数据表\n" +
     ...
     * 测试远程执行JAR包的功能
     * <p>
     * 该测试方法验证Flink远程执行器能否通过REST API正确提交并执行指定的JAR文件。
     * 主要包括设置远程Flink集群连接配置、指定JAR文件路径等操作。
     * <p>
     * 测试流程：
     * 1. 创建Flink远程执行器实例
     * 2. 构造远程执行参数，包括连接配置和JAR文件信息
     * 3. 设置Flink集群的REST服务地址和端口
     * 4. 指定要执行的JAR文件路径
     * 5. 调用执行器向远程集群提交JAR任务
     *
    ...
    void executeJar() {
        ...
        // 设置要执行的JAR文件路径
        executeParam.setJarFile("F:\\Users\\yzh\\IdeaProjects\\zt\\yudao-boot-mini-zt\\yudao-module-flink\\yudao-flink-common-executor\\src\\test\\resources\\WordCount.jar");
        ...
        // 执行远程JAR包提交
        executor.deployJar(executeParam);
    }
